...
func NewTimeUUID() string {
	return TimeUUID().String()
}

func NewUUID() string {
	if id, err := RandomUUID(); err != nil {
		panic(errors.Wrap(err, "Unable to generate UUID"))
	} else {
		return id.String()
	}
}
...
func ValidUUID(id string) error {
	if id == "" {
		return errors.New("Invalid UUID - empty string")
	}

	_, err := ParseUUID(id)
	return errors.Wrapf(err, "Invalid UUID '%v'", id)
}

func MustParseUUID(id string) UUID {
	u, err := ParseUUID(id)

	if err != nil {
		panic(errors.Wrapf(err, "Unable to parse UUUID '%v' - should be checked before this stage in code", id))
	}
	return u
}
